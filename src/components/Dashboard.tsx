...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div id="dashboard-page" className="bg-slate-50 min-h-screen text-slate-800 font-sans selection:bg-amber-100 flex flex-col">
      {/* Top Admin Nav Bar */}
      <header id="dashboard-header" className="bg-white border-b border-slate-200/80 sticky top-0 z-40 px-4 py-3.5">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              id="back-to-landing-btn"
              onClick={() => setView('landing')}
              className="p-2 -ml-2 hover:bg-slate-100 rounded-xl text-slate-400 hover:text-slate-600 transition-colors cursor-pointer"
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-black text-lg tracking-tight text-slate-900 hidden sm:inline">
                {t.dashboardTitle}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {currentUser && (
              <span className="text-slate-500 text-xs hidden sm:inline-block font-mono bg-slate-100 px-3 py-1.5 rounded-lg">
                👤 @{currentUser}
              </span>
            )}
            <button
              ...
              className="bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-all shadow-sm flex items-center gap-1.5 cursor-pointer"
            >
            ...
            {currentUser && (
              <button
                id="logout-btn"
                onClick={logoutUser}
                className="text-red-500 hover:text-red-600 hover:bg-red-50 font-bold text-xs px-3 py-2.5 rounded-xl transition-colors cursor-pointer"
              >
                Logout
              </button>
            )}
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="flex items-center gap-4 bg-white border border-slate-200/60 rounded-2xl p-4 shadow-sm">
              <div className="w-12 h-12 bg-amber-50 text-amber-600 rounded-2xl flex items-center justify-center shrink-0">
                <Package className="w-6 h-6" />
              </div>
              <div>
                <span className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Products</span>
                <div className="font-black text-2xl text-slate-900 leading-none mt-1">{totalProducts}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 bg-white border border-slate-200/60 rounded-2xl p-4 shadow-sm">
              ...
              <div>
                <span className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Categories</span>
                <div className="font-black text-2xl text-slate-900 leading-none mt-1">{totalCategories}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 bg-white border border-slate-200/60 rounded-2xl p-4 shadow-sm">
              ...
              <div>
                <span className="text-[11px] text-slate-400 font-bold uppercase tracking-wider">Currency</span>
                <div className="font-black text-2xl text-slate-900 leading-none mt-1">{settings.currencySymbol}</div>
              </div>
            </div>
          </div>
